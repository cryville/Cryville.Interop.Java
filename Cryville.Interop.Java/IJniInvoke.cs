namespace Cryville.Interop.Java {
	...
	public interface IJniInvoke {
		...
		/// </summary>
		/// <returns><see cref="JniResult.OK" /> on success; a suitable JNI error code (a negative number) on failure.</returns>
		/// <remarks>
		...
		/// </remarks>
		JniResult AttachCurrentThread(out IJniEnv p_env, JavaVMAttachArgs? thr_args);
		/// <summary>
		...
		/// <returns><see cref="JniResult.Detached" /> if the current thread is not attached to the VM; <see cref="JniResult.Version" /> if the specified version is not supported; <see cref="JniResult.OK" /> otherwise.</returns>
		JniResult GetEnv(out IJniEnv env, int version);
		...
		/// </remarks>
		JniResult AttachCurrentThreadAsDaemon(out IJniEnv penv, JavaVMAttachArgs? args);
	}
}
